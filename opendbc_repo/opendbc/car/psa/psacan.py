...
def create_lane_messages(packer, lat_active: bool, apply_curvature: float, heading: float):
  # Lane line messages normally sent by the LKAS camera. The car-side LKA ECU derives its
  # steering angle request (LANE_KEEP_ASSIST) from them. We inject curvature plus a synthesized
  # heading (the ECU steers mostly on heading at speed). Heading shares the curvature sign.
  # TODO: verify LINE_CURVATURE / LINE_HEADING sign on car, camera convention is opposite of openpilot
  msgs = []
  for name, lat_pos in (('LKAS_CAM_LANE_LEFT', 1.75), ('LKAS_CAM_LANE_RIGHT', -1.75)):
    values = {
      'LINE_HEADING': -heading if lat_active else 0.,
